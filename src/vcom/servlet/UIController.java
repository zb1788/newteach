...
public class UIController extends HttpServlet {
    ...
    public void init(ServletConfig config) {
        if (configMap == null) {
            logger.debug("InterfaceCfg INIT in UIController!!!");
            //�ȼ�������
            vcom.util.InterfaceCfg.getSysPath("PLS");
            logger.debug("INIT UIController!!!");
            getConfigs();
        }
    }
    ...
    private void controller(HttpServletRequest request, HttpServletResponse response, String method) {
        ...
        if (tconfig == null) {
            try {
                response.setContentType("text/html;charset=" + Config.ENCODING);
                //response.setStatus(404);
                ServletOutputStream out = response.getOutputStream();
                out.write(("UICONFIG ERROR:CAN NOT GET CONFIG !").getBytes(Config.ENCODING));
            ...
            try {
                ...
                if (resultStr != null && resultStr.trim().length() > 0 && tconfig.getResult(resultStr) != null) {
					...
					try{
						...
						ServletOutputStream out = response.getOutputStream();
						out.write("UICONFIG ERROR:CAN NOT GET RESULT CONFIG!".getBytes(Config.ENCODE));
					} catch (UnsupportedEncodingException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					} catch (IOException e) {
    ...
    private void getConfigs() {
        InputStream inf = UIController.class.getResourceAsStream("/uiconfig.xml");
        if (inf == null) {
